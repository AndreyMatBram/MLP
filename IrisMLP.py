...
def test_learning_rates(X, y, X_test, y_test, minLr= 0.001, maxLr=0.5, seed=42):
    results = []
    lr = minLr

    while lr < maxLr:
        mlp = MLP(input_size=4, hidden_size=5, output_size=3, learning_rate=lr, random_seed=seed)
        mlp.train(X, y, epochs=500)
        predictions = mlp.predict(X_test)
        y_true = np.argmax(y_test, axis=1)
        accuracy = np.mean(predictions == y_true)
        results.append((lr, accuracy, mlp.weights_input_hidden.copy(), mlp.weights_hidden_output.copy()))
        lr += minLr
    
    return results

def plot_accuracy_vs_lr(results):
    ...
    plt.title('Relação entre Taxa de Aprendizado e Acurácia')
    plt.xscale('log')
    plt.grid(True)
    plt.show()
...
def main():
    ...
    # input_size = 4 (4 atributos de entrada, como comprimento e largura da sépala e pétala)
    # hidden_size = Numero de neurônios na camada oculta (0 significa sem camada oculta)
    # output_size = 3 (3 classes de saída, como Iris-setosa, Iris-versicolor e Iris-virginica) 
    mlp = MLP(input_size=4, hidden_size=6, output_size=3, learning_rate=lr, random_seed=randSeed)

    # Treinando a rede neural e plotando a perda
    history = mlp.train(data_train, output_train, epochs=1000)

    plot_loss(history, title='Gráfico de perda SEM normalização')
    ...
    datasets = [(data_train, output_train), 
                (data_validation, output_validation), 
                (data_test, output_test)]


    for (data_split, output_split) in datasets:
        output_true = np.argmax(output_split, axis=1)
        output_pred = mlp.predict(data_split)
        accuracy = np.mean(output_true == output_pred)
        accuracies.append(accuracy)
        cm = confusion_matrix(output_true, output_pred)
        conf_matrices.append(cm)
    
    plot_all_matriz_confusao(conf_matrices, accuracies, split_names, title='Matriz de Confusão SEM normalização')  # Gráfico de matriz de confusão SEM normalização
    ...
    mlp_norm = MLP(input_size=4, hidden_size=6, output_size=3, learning_rate=lr, random_seed=randSeed)
    ...
    testsResults =  test_learning_rates(data_train, output_train, data_test, output_test, minLr=0.001, maxLr=0.5, seed=randSeed)
    plot_accuracy_vs_lr(testsResults) 
