...
def load_data():
    ...
    df = pd.read_csv('wine.data', header=None, sep=',')
...
def test_learning_rates(X, y, X_test, y_test, minLr= 0.001, maxLr=0.5, seed=42):
    ...
    while lr < maxLr:
        mlp = MLP(input_size=13, hidden_size=6, output_size=3, learning_rate=lr, random_seed=seed)
...
def main():
    ...
    for i in range(len(results)):
        output[i][results[i]-1] = 1 # 0, 1, 2 => [1, 0, 0], [0, 1, 0], [0, 0, 1]
    ...
    mlp_norm = MLP(input_size=13, hidden_size=6, output_size=3, learning_rate=lr, random_seed=randSeed)
